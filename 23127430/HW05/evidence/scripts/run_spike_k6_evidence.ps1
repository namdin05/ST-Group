param(
    ...
    [ValidateSet('dry-run', 'spike')]
    [string]$Mode
)

$ErrorActionPreference = 'Stop'
$state = Get-Content -Raw -LiteralPath $StatePath | ConvertFrom-Json
$script = Join-Path $state.repo 'test-plans\spike\23127430_Spike_20260815.js'
$rawDir = Join-Path $state.resultDir 'raw'
$htmlDir = Join-Path $state.resultDir 'html'
$baseName = if ($Mode -eq 'dry-run') {
    "23127430_Spike_$($state.runId)_dry-run"
} else {
    "23127430_Spike_$($state.runId)"
}
...
$Host.UI.RawUI.WindowTitle = if ($Mode -eq 'dry-run') {
    'HW05 k6 Spike Dry Run Evidence'
} else {
    'HW05 k6 Full Spike Evidence'
}
...
$passed = $k6Exit -eq 0 -and $httpThresholdPassed -and
    ...
    $metrics.checks.fails -eq 0

$verdict = if ($passed) { 'PASS' } else { 'FAIL' }
$verdict | Set-Content -LiteralPath $marker -Encoding ascii

Clear-Host
Write-Host 'HW05 - k6 SPIKE EXECUTION EVIDENCE' -ForegroundColor Cyan
