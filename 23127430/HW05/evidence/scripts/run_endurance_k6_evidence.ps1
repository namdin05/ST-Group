param(
    ...
    [ValidateSet('dry-run', 'endurance')]
    [string]$Mode
)

$ErrorActionPreference = 'Stop'
$state = Get-Content -Raw -LiteralPath $StatePath | ConvertFrom-Json
$script = Join-Path $state.repo 'test-plans\endurance\23127430_Endurance_20260815.js'
$rawDir = Join-Path $state.resultDir 'raw'
$htmlDir = Join-Path $state.resultDir 'html'
$baseName = if ($Mode -eq 'dry-run') {
    "23127430_Endurance_$($state.runId)_dry-run"
} else {
    "23127430_Endurance_$($state.runId)"
}
...
$Host.UI.RawUI.WindowTitle = if ($Mode -eq 'dry-run') {
    'HW05 k6 Endurance Dry Run Evidence'
} else {
    'HW05 k6 Full Endurance Evidence'
}

Set-Location -LiteralPath $state.repo
if ($Mode -eq 'endurance') {
...
$httpThresholdPassed = $metrics.http_req_failed.value -lt 0.01 -and
...
$latencyThresholdPassed = $metrics.http_req_duration.'p(95)' -lt 500 -and
    -not $metrics.http_req_duration.thresholds.'p(95)<500'
$checksThresholdPassed = $metrics.checks.value -eq 1 -and
    -not $metrics.checks.thresholds.'rate==1'
...
Clear-Host
Write-Host 'HW05 - k6 ENDURANCE EXECUTION EVIDENCE' -ForegroundColor Cyan
Write-Host "Profile: $Mode"
Write-Host "k6 exit code: $k6Exit"
Write-Host "HTTP failure threshold (<1%): $(if ($httpThresholdPassed) {'PASS'} else {'FAIL'})"
Write-Host "Functional threshold (0%): $(if ($functionalThresholdPassed) {'PASS'} else {'FAIL'})"
Write-Host "Latency threshold (p95 < 500 ms): $(if ($latencyThresholdPassed) {'PASS'} else {'FAIL'})"
Write-Host "Check-rate threshold (100%): $(if ($checksThresholdPassed) {'PASS'} else {'FAIL'})"
Write-Host ''
