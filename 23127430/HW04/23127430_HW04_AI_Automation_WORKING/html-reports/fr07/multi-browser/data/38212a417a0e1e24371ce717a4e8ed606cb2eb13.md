...
- Name: fr07-shopping-cart.spec.ts >> FR-07 – Shopping Cart >> UI cases >> TC004 – plus control increments quantity and totals
- Location: tests\fr07-shopping-cart.spec.ts:280:9

# Error details

```
Error: Each cart row requires a plus control
...
Call log:
  - Each cart row requires a plus control with timeout 5000ms
...
Locator:  getByRole('table').getByRole('row').filter({ hasText: 'Laptop Gaming Pro' }).getByRole('cell').nth(2)
Expected: "2"
Received: "1"
Timeout:  5000ms
...
Call log:
  ...
  - waiting for getByRole('table').getByRole('row').filter({ hasText: 'Laptop Gaming Pro' }).getByRole('cell').nth(2)
    11 × locator resolved to <td>1</td>
       - unexpected value "1"
