...
- Name: fr01-registration.spec.ts >> FR-01 – Account Registration >> UI cases >> TC003 – invalid email format is rejected
- Location: tests\fr01-registration.spec.ts:191:9

# Error details

```
Error: Email should use native email semantics

expect(locator).toHaveAttribute(expected) failed

Locator:  locator('form').filter({ has: getByRole('button', { name: 'Đăng Ký', exact: true }) }).locator('input[type="text"]').nth(1)
Expected: "email"
Received: "text"
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('form').filter({ has: getByRole('button', { name: 'Đăng Ký', exact: true }) }).locator('input[type="text"]').nth(1)
    11 × locator resolved to <input required="" type="text" value="newuser.example.com" class="w-full border p-2 rounded"/>
...
Locator: getByText(/email.*(?:không hợp lệ|invalid)|(?:không hợp lệ|invalid).*email/i)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText(/email.*(?:không hợp lệ|invalid)|(?:không hợp lệ|invalid).*email/i)
