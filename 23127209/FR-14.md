...
---
  ...
  **Nguyên tắc áp dụng:** "Must be" (bắt buộc, không được để trống)
  ...
  | **EC10** | ❌ Invalid | Tên danh mục chỉ chứa khoảng trắng (VD: `"   "`) | Chia nhỏ — bypass validation "không rỗng" nếu backend không trim. **Spec ambiguity** |
...
---
  ...
  | **TC-V2** | Xóa danh mục thành công | Vai trò: Admin, danh mục cần xóa tồn tại trong DB | Chọn danh mục → Bấm Xóa | EC3, EC5 | ✅ Danh mục bị xóa khỏi DB, danh sách không còn hiển thị danh mục đã xóa |
  ...
  | **TC-I2** | Non-admin thử thao tác danh mục | Vai trò: Non-admin (user thường) | Thử truy cập / thao tác quản lý danh mục (Thêm/Xóa) | **EC4** | Bị từ chối, không thể thao tác. *(Ghi chú: Spec ambiguity — Non-admin có thể "Xem" danh mục không? Cần verify thực tế)* |
  ...
  | **TC-I5** | Thêm danh mục tên chỉ có khoảng trắng | Vai trò: Admin | Tên = `"   "` (chỉ spaces) → Bấm Thêm | **EC10** | ⚠️ **Spec ambiguity** — Cần test thực tế: (1) Hệ thống trim → coi rỗng → từ chối, hoặc (2) Không trim → chấp nhận tên vô nghĩa |
  ...
  | EC3 | ✅ Valid | Vai trò Admin | TC-V1, TC-V2 |
  ...
  | EC9 | ❌ Invalid | Tên danh mục trùng | TC-I4 |
  | EC10 | ❌ Invalid | Tên chỉ có khoảng trắng | TC-I5 |
  ...
  | Invalid Test Cases | **5** | **6/6** Invalid ECs ✅ |
  | **Tổng** | **7** | **10/10 (100%)** |
...
---
  ...
  | **TC-V2** | Xóa danh mục thành công | Valid | EC3, EC5 |
  ...
  | **TC-I5** | Thêm danh mục tên chỉ khoảng trắng | Invalid | EC10 |
  ...
  | Invalid (Bước 3) | TC-I1 → TC-I5 | 5 |
  | BVA (Bước 4) | TC-BVA1 | 1 |
  | **Tổng FR-14** | | **8 test cases** |
  | **ECs coverage** | | **10/10 (100%)** |
