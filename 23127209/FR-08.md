...
---
  ...
  **Nguyên tắc áp dụng:** "Must be" (Không chấp nhận giá trị do client gửi lên).
  ...
  | EC ID | Loại | Phân hoạch | Nguyên tắc |
  |---|---|---|---|
  | EC3 | ✅ Valid | Client gửi payload (có hoặc không chứa `total_amount`) — Backend **luôn tự tính lại** tổng tiền, bỏ qua giá trị client | Must be — thỏa mãn |
  | EC4 | ❌ Invalid | Client cố tình gửi `total_amount` sai lệch (VD: sửa thành 1đ) — Backend vẫn phải tự tính, **không** dùng giá trị client | Must be — không thỏa mãn (Test bảo mật) |
...
## Bước 3: Lựa chọn tập Test Cases tối thiểu
  ...
  *(Ghi chú: EC13 - "Giỏ hàng giữ nguyên khi thất bại" là một Valid EC nhưng đối nghịch với Happy Path, nó sẽ được tích hợp kiểm chứng vào các kịch bản Invalid bên dưới).*
  ...
  |---|---|---|---|---|---|---|
  | **TC-I1** | Cố tình can thiệp UI tổng tiền | S1: Đã đăng nhập, S2: Có SP | V1: Dùng DevTools sửa DOM form tổng tiền, V2: Gửi payload đúng | **EC2** | **Kịch bản A:** Frontend chặn submit → không tạo đơn (verify EC18). **Kịch bản B:** Backend bỏ qua giá trị sửa, tạo đơn đúng giá thật (verify EC17, EC9). *(Cần xác định qua test thực tế)* | EC13 |
...
## Tổng hợp
  ...
  | **TC-I1** | Cố tình can thiệp UI tổng tiền | Invalid | EC2, verify EC13 |
  ...
  | BVA (Bước 4) | TC-BVA1 → TC-BVA3 | 3 |
  | **Tổng FR-08** | | **8 test cases** |
  | **ECs coverage** | | **18/18 (100%)** |
  | **BVs coverage** | | **6/6 (100%)** |